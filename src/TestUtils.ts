 ...
 */
export function getLastEmittedEvent<
  Ev extends EventNames<ServerToClientEvents>
>(
...
>(
  ...
  const ret = mockSocket.on.mock.calls.find(
    (eachCall) => eachCall[0] === eventName
  );
  if (ret != null) {
    const param = ret[1];
...
export function mockPlayer(townID: string): MockedPlayer {
  ...
  const socketToRoomMock =
    mock<BroadcastOperator<ServerToClientEvents, SocketData>>();
  socket.to.mockImplementation((room: string | string[]) => {
    if (townID === room) {
      return socketToRoomMock;
    }
    throw new Error(
      `Tried to broadcast to ${room} but this player is in ${townID}`
    );
  });
 ...
 * @param expected
 */
export function expectArraysToContainSameMembers<T>(
  actual: T[],
  expected: T[]
): void {
